...
object RNG {
  ...
  def ints(count: Int)(rng: RNG): (List[Int], RNG) = {
    @tailrec def loop(count: Int,
                      rng: RNG,
                      acc: List[Int]): (List[Int], RNG) = {
      if (count == 0) (acc, rng)
      else {
        val (i, r) = rng.nextInt
  ...
  // 6.5
  def doubleThroughMap: Rand[Double] =
  ...
  def sequence[A](as: List[Rand[A]]): Rand[List[A]] =
    as.foldRight(unit(List.empty[A]))( (r, acc) => map2(r, acc)(_ :: _))
